...
class Block(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mambax: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        # print('到block模块')
        shortcut = x
        x = self.norm1(x)
        # Window partition
        ...
        if self.window_size == 0:
            sax = self.Space_Adapter(x, mambax, mambax)  # b h w c
            x = x + sax
            mambax = self.refine_Adapter(mambax.permute(0, 3, 1, 2)).permute(0, 2, 3, 1)
